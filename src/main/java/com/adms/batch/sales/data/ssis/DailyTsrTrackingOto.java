...
public class DailyTsrTrackingOto {
...
//		test("D:/Work/Report/DailyReport/201502");
			...
			throws Exception
	{
		...
		fw.walk(sInputPath, new FilenameFilter()
		{
			...
			{
				return !name.contains("~$") && (dir.getAbsolutePath().contains("OTO") && name.toUpperCase().contains("TSR") && name.toUpperCase().contains("TRACKING"));
			}
		});
		...
		for (String fileName : fw.getFileList())
		{
			...
			new DailyTsrTrackingFileTransform().transform("FileFormat_SSIS_DailyTsrTracking-input-OTO.xml", new File(fileName), "FileFormat_SSIS_DailyTsrTracking-output.xml", new File(outputFileNameSsis));
		}
	}
